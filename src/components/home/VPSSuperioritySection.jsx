...
import { Terminal, CheckCircle2, ArrowRight, ShieldCheck, Server, Cpu, HardDrive, Network, Sparkles, Key, Box } from 'lucide-react';
  ...
  return (
    <section ref={sectionRef} id="vps-hosting" data-slot="freedom" className="py-24 bg-white text-[rgb(26,11,84)] relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* 5-Column VPS Plan Showcase Grid */}
        <div ref={cardsContainerRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-16">
          {VPS_PLANS.map((plan) => {
            ...
            return (
              <div
                key={plan.id}
                className={`relative rounded-3xl p-5 sm:p-6 flex flex-col justify-between transition-all duration-300 hover:scale-105 ${
                  plan.highlight
                    ? 'bg-white border-2 border-blue-600 shadow-2xl lg:-translate-y-2'
                    : 'bg-white border border-slate-200 hover:border-slate-300 shadow-lg'
                }`}
              >
                {/* Popular Badge */}
                {plan.badge && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-0.5 rounded-full bg-blue-600 text-white text-[9px] font-mono font-bold uppercase tracking-[0.14em] whitespace-nowrap shadow-md">
                    {plan.badge}
                  </div>
                )}
                ...
                <div className="space-y-4">
                  ...
                  <div className="p-2 rounded-xl bg-slate-50 border border-slate-200 text-[11px] font-mono text-slate-700 flex items-center justify-between">
                  ...
                  <div className="py-2.5 border-y border-slate-100">
                    <div className="flex items-baseline space-x-1">
                      ...
                      <span className="text-3xl sm:text-4xl font-bold font-display text-[rgb(26,11,84)]">
                        {displayPrice}
                      </span>
